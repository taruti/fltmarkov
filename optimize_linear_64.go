...
type OptimizeLinear64 [64]float32
func (r *OptimizeLinear64) initial(data []float32) {
 ...
 scalef := (max - min) / float32(64)
 for i := range r {
  r[i] = float32(i) * scalef
 }
}
func (r *OptimizeLinear64) Generate(data []float32) {
 r.initial(data)
 var counts [64]int32
 var necounts [64]int32
outer:
 for i := 0; i < 10*64; i++ {
  counts = [64]int32{}
  necounts = [64]int32{}
...
func (r *OptimizeLinear64) linearBin(d float32) (int32, bool) {
 min := float32(math.Inf(1))
 bin := int32(0)
 for i, x := range r {
  v := absF32(d - x)
